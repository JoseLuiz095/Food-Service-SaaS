...
export default function MasterPlans() {
  ...
  const { plans, loading, savePlan, updatePlatformSettings, platformSettings } = useMaster();
  ...
  useEffect(() => {
    setDraftPlans([...plans].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)));
  }, [plans]);

  useEffect(() => {
    setDemoEnabled(platformSettings.demoEnabled ?? true);
    setDemoDays(platformSettings.demoDurationDays ?? 7);
    setDemoWarningDays(platformSettings.demoWarningDays ?? 2);
  }, [platformSettings.demoEnabled, platformSettings.demoDurationDays, platformSettings.demoWarningDays]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const settings = await platformApi.getPlatformSettings();
        if (mounted) setBilling(settings);
      } catch (error) {
  ...
  if (loading || !billing) return <LoadingState label="Carregando planos..." />;
  ...
  const saveBilling = async () => {
    ...
    try {
      await trackInteraction('master_save_billing_config', () => updatePlatformSettings(billing), { scope: 'master' });
  ...
  const savePlanCard = async (plan: Plan) => {
    ...
    try {
      await trackInteraction('master_plan_save', () => savePlan(plan.id, plan), { scope: 'master', planCode: plan.code });
    ...
    <>
      ...
      <section className="master-settings-pair">
        ...
        <section className="admin-card food-master-billing-card">
          ...
          <p>
            Cadastro usado para gerar o copia e cola com valor dinâmico das mensalidades do FoodWeb.
          </p>
          ...
          {(billing.billingPixCopyPaste ?? '').trim() && (
            <div className="food-master-billing-preview">
              ...
              <button
                ...
                onClick={() => void copy(billing.billingPixCopyPaste ?? '').then(() => showToast('PIX copiado.', 'success'))}
              >
                <Copy size={15} />
                Copiar base
              </button>
          ...
          return (
            <article className={`admin-card master-plan-card ${featured ? 'featured' : ''} ${isDemo ? 'is-demo' : ''}`} key={plan.id}>
              ...
              <div className="master-plan-features">
                <label>
                  <input type="checkbox" checked={planHasFeature(plan, 'analytics')} onChange={(event) => updatePlan(plan.id, { analytics: event.target.checked })} />
                  Analytics
                </label>
                <label>
                  <input type="checkbox" checked={planHasFeature(plan, 'finance')} onChange={(event) => updatePlan(plan.id, { finance: event.target.checked })} />
                  Financeiro
                </label>
                <label>
                ...
                <label>
                  <input type="checkbox" checked={planHasFeature(plan, 'custom_banner')} onChange={(event) => updatePlan(plan.id, { customBanner: event.target.checked })} />
      ...
      <section className="admin-card food-business-plan-v051 unified-business-card">
