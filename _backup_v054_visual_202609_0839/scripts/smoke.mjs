...
ok('Pacote FoodWeb v0.5.3',pkg.name==='foodservice-saas'&&pkg.version==='0.5.3');
...
console.log(`\nSmoke FoodWeb v0.5.3 concluído: ${checks.length} verificações + ${sourceFiles.length} arquivos TS/TSX.`);


// FoodWeb v0.5.3 - paridade visual com FloriWeb no Admin/Admin Master
ok('v0.5.3 Admin usa agrupamento visual do FloriWeb',adminLayout.includes("label: 'Operação'")&&adminLayout.includes("label: 'Catálogo'")&&adminLayout.includes("label: 'Gestão'")&&adminLayout.includes("label: 'Conta'"));
ok('v0.5.3 Admin preserva regras Food por feature',adminLayout.includes('requiresFeature')&&adminLayout.includes('planHasFeature'));
ok('v0.5.3 Admin preserva navegacao para Admin Master',adminLayout.includes('platformAdmin')&&adminLayout.includes('/admin-master'));
ok('v0.5.3 visual possui marcador de paridade',css.includes('FoodWeb v0.5.3 — paridade visual com FloriWeb'));
ok('v0.5.3 visual mantem identidade alimentar',css.includes('--food-admin-accent:#c85d37')&&css.includes('--food-admin-sidebar:#3b251f'));
ok('v0.5.3 Admin Master usa mesmo sistema de cards',css.includes('.master-shell .admin-content')&&css.includes('.master-admin-mini'));
