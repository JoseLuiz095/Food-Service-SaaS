...
const DEFAULT_PLATFORM_SETTINGS: PlatformSettings = { demoEnabled:true,demoDurationDays:14,demoWarningDays:3,billingProvider:'manual',billingPixKeyType:'',billingPixKey:'',billingPixHolderName:'',billingPixCopyPaste:'',billingWhatsapp:'',billingProofRequired:true,billingAutoRenew:false,billingGraceDays:3 };
...
type PlatformSettingsRow = { id:number; demo_enabled?:boolean; demo_duration_days:number; demo_warning_days:number; billing_provider?:'manual'|'asaas'; billing_pix_key_type?:string|null; billing_pix_key?:string|null; billing_pix_holder_name?:string|null; billing_pix_copy_paste?:string|null; billing_whatsapp?:string|null; billing_proof_required?:boolean; billing_auto_renew?:boolean; billing_grace_days?:number|null };
...
const mapPlatformSettings = (row?: PlatformSettingsRow): PlatformSettings => row ? {
  ...
  billingProvider:row.billing_provider||'manual', billingPixKeyType:row.billing_pix_key_type||'', billingPixKey:row.billing_pix_key||'', billingPixHolderName:row.billing_pix_holder_name||'', billingPixCopyPaste:row.billing_pix_copy_paste||'', billingWhatsapp:row.billing_whatsapp||'', billingProofRequired:row.billing_proof_required!==false, billingAutoRenew:row.billing_auto_renew===true, billingGraceDays:row.billing_grace_days??3,
...
const demoPlans: Plan[] = [
  {id:'demo',code:'DEMO',name:'Teste grátis',productLimit:null,imageLimitPerProduct:10,customDomain:false,reports:true,prioritySupport:true,monthlyPrice:0,setupPrice:0,categoryLimit:null,addonLimit:null,adminUserLimit:5,sortOrder:0,active:true,featureCodes:['catalog','orders','whatsapp','delivery','billing_pix','analytics','custom_banner','finance','multi_user'],featureLimits:{}},
  {id:'essential',code:'ESSENTIAL',name:'Essencial',productLimit:40,imageLimitPerProduct:1,customDomain:false,reports:false,prioritySupport:false,monthlyPrice:49.9,setupPrice:0,categoryLimit:12,addonLimit:40,adminUserLimit:2,sortOrder:10,active:true,featureCodes:['catalog','orders','whatsapp','delivery','billing_pix'],featureLimits:{}},
  {id:'starter',code:'STARTER',name:'Starter',productLimit:120,imageLimitPerProduct:5,customDomain:false,reports:true,prioritySupport:false,monthlyPrice:79.9,setupPrice:0,categoryLimit:30,addonLimit:120,adminUserLimit:4,sortOrder:20,active:true,featureCodes:['catalog','orders','whatsapp','delivery','billing_pix','analytics','custom_banner'],featureLimits:{}},
  {id:'professional',code:'PROFESSIONAL',name:'Profissional',productLimit:null,imageLimitPerProduct:10,customDomain:false,reports:true,prioritySupport:true,monthlyPrice:119.9,setupPrice:0,categoryLimit:null,addonLimit:null,adminUserLimit:8,sortOrder:30,active:true,featureCodes:['catalog','orders','whatsapp','delivery','billing_pix','analytics','custom_banner','finance','multi_user'],featureLimits:{}},
];
...
export const platformApi = {
  ...
  async savePlatformSettings(settings: PlatformSettings): Promise<PlatformSettings> {
    const normalized: PlatformSettings = {
      ...
      billingProvider:settings.billingProvider||'manual', billingPixKeyType:settings.billingPixKeyType.trim(), billingPixKey:settings.billingPixKey.trim(), billingPixHolderName:settings.billingPixHolderName.trim(), billingPixCopyPaste:settings.billingPixCopyPaste.trim(), billingWhatsapp:settings.billingWhatsapp.replace(/\D/g,''), billingProofRequired:settings.billingProofRequired!==false, billingAutoRenew:settings.billingProvider==='asaas', billingGraceDays:Math.max(0,Math.min(30,Math.round(settings.billingGraceDays||0))),
    };
    if (isDemoMode) return normalized;
    const rows = await restFetch<PlatformSettingsRow[]>('food_platform_settings?id=eq.1&select=*', {
      method:'PATCH',
      body:{demo_enabled:normalized.demoEnabled,demo_duration_days:normalized.demoDurationDays,demo_warning_days:normalized.demoWarningDays,billing_provider:normalized.billingProvider,billing_pix_key_type:normalized.billingPixKeyType||null,billing_pix_key:normalized.billingPixKey||null,billing_pix_holder_name:normalized.billingPixHolderName||null,billing_pix_copy_paste:normalized.billingPixCopyPaste||null,billing_whatsapp:normalized.billingWhatsapp||null,billing_proof_required:normalized.billingProofRequired,billing_auto_renew:normalized.billingAutoRenew,billing_grace_days:normalized.billingGraceDays},
  ...
  async listSubscriptionPayments(): Promise<SubscriptionPayment[]> {
    if(isDemoMode)return [];
    const [rows,stores,plans]=await Promise.all([restFetch<PaymentRow[]>('food_subscription_payments?select=*&order=created_at.desc&limit=200'),restFetch<StoreRow[]>('food_stores?select=id,name'),restFetch<PlanRow[]>('food_plans?select=id,name')]);
    return rows.map((row)=>({id:row.id,storeId:row.store_id,subscriptionId:row.subscription_id||undefined,planId:row.plan_id,amount:toNumber(row.amount),dueDate:row.due_date,provider:row.provider,providerPaymentId:row.provider_payment_id||undefined,status:row.status,pixPayload:row.pix_payload||undefined,pixQrBase64:row.pix_qr_base64||undefined,providerExpirationAt:row.provider_expiration_at||undefined,proofRequired:row.proof_required,proofSentAt:row.proof_sent_at||undefined,paidAt:row.paid_at||undefined,createdAt:row.created_at,storeName:stores.find((s)=>s.id===row.store_id)?.name,planName:plans.find((p)=>p.id===row.plan_id)?.name}));
  },
