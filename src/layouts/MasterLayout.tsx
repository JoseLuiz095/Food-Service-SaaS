...
export default function MasterLayout(){
  ...
  return <div className="admin-shell master-shell">
    ...
    <aside className={`admin-sidebar master-sidebar ${open?'is-open':''}`}>
      ...
      <div className="admin-brand"><ShieldCheck size={24}/><div><strong>Food Service SaaS</strong><span>Gestão da plataforma</span></div></div>
    ...
    <section className="admin-content">
      <header className="admin-topbar"><div><span className="eyebrow">ADMIN MASTER</span><strong>Gestão central dos estabelecimentos</strong></div><div className="admin-user"><span>{(platformAdmin?.name||'FW').slice(0,2).toUpperCase()}</span><div><strong>{platformAdmin?.name||'Administrador da plataforma'}</strong><small>{user?.email}</small></div></div></header>
