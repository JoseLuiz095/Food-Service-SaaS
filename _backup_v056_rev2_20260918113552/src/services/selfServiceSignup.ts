...
export async function completeFoodSelfServiceSignup(input?: Partial<SelfServiceSignupInput>) {
  return rpc<SelfServiceCompletion>('food_complete_self_service_signup_v1', completionBody(input));
}
...
export async function listFoodSelfServiceSignupRequests(): Promise<SelfServiceSignupRequest[]> {
  const rows = await rpc<SignupRow[]>('food_platform_list_self_service_signups_v1');
  return (rows || []).map((row) => ({
    id: row.id,
...
export const approveFoodSelfServiceSignup = (requestId: string) =>
  rpc<{ ok: boolean; storeId?: string }>('food_platform_approve_self_service_signup_v1', { p_request_id: requestId });
