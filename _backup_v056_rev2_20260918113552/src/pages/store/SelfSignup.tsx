...
export default function SelfSignup(){
  ...
  const[form,setForm]=useState<SelfServiceSignupInput>({
    storeName:'',ownerName:'',email:'',password:'',planCode:(searchParams.get('plan')||'DEMO').toUpperCase(),city:'',state:'',captchaToken:'',
  });
    ...
    <main className="self-signup-shell">
      <section className="self-signup-intro">
        ...
        <div className="self-signup-rules">
          <article><Clock3/><div><strong>Demo por {demoDays} dias</strong><span>Disponível uma única vez por e-mail no FoodWeb.</span></div></article>
      ...
      <section className="self-signup-card">
        ...
        <form onSubmit={submit}>
          ...
          {isDemo&&demoEnabled&&<div className="self-signup-notice"><Clock3 size={17}/><span>Os dias do Demo começam somente após a aprovação do Admin Master e não podem ser concedidos novamente ao mesmo e-mail.</span></div>}
          ...
          <small className="self-signup-footnote">Ao continuar, você solicita a criação de um ambiente FoodWeb. Cadastros podem ser revisados pelo Admin Master antes da liberação definitiva.</small>
        </form>
      </section>
    </main>
  </div>;
